      ...
      <div ref={containerRef} className="mobile-alt-light scroll-smooth">
        {copy.collections.map((collection, index) => (
          <section
            ...
            ref={setRef(index)}
            data-index={index}
            className={`flex min-h-screen w-full snap-start flex-col ${index === 0 ? "" : "border-t border-border/60"} pb-12 md:h-screen md:overflow-hidden`}
          >
            {/* Editorial hero band (top ~30% of every section, PC/tablet + first section on mobile) */}
            <div
              className={`relative shrink-0 items-center overflow-hidden pb-6 md:flex md:h-[30%] ${
                index === 0 ? "flex h-[30%]" : "hidden"
              }`}
            >
                <img
                  src={COLLECTION_IMAGES[collection.slug] ?? colSweet}
            ...
            <div className={`mx-auto flex min-h-0 w-full max-w-[1400px] flex-1 flex-col justify-start px-6 md:px-10 md:pt-0 ${index === 0 ? "pt-0" : "pt-24"}`}>
